 ...
 * could not be found in the database. It uses the `ResponseTrait` to ensure
...
class ModelNotFoundException
{
    ...
    public function render()
    {
        ...
        return $this->responseNotFound(null, $this->getMessage());
    }
    ...
    {
        // Access the exception property correctly.
        $modelClass = class_basename($this->exception->getModel());
        $modelId = $this->exception->getIds() ? implode(',', $this->exception->getIds()) : 'unknown';
        
        // Return a localized error message with model class and ID
        return __('LaravelAPIKit::message.model_not_found', ['model' => $modelClass, 'id' => $modelId]);
    }
}
